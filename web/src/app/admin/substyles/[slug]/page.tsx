...
import { getAdminSubstyleDetailBySlug } from "@/server/db/admin-taxonomy";
import { getStylesCatalogWithOptions } from "@/server/db/catalog";
import { buildAdminHref } from "../../_helpers";

type SubstyleDetailSearchParams = Promise<{
  success?: string;
  error?: string;
}>;

export default async function AdminSubstyleDetailPage({
  params,
  searchParams,
}: {
  params: Promise<{ slug: string }>;
  searchParams: SubstyleDetailSearchParams;
}) {
  ...
  const [detail, styles, mediaLinks] = await Promise.all([
    getAdminSubstyleDetailBySlug(slug),
  ...
  const redirectTo = buildAdminHref(`/admin/substyles/${detail.substyle.slug}`, role);
        ...
        <section className="min-w-0 flex-1 space-y-6">
          <header className="rounded-2xl border border-[var(--border-1)] bg-[var(--surface-1)] p-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-3xl font-bold text-white">Editar Substyle</h2>
                <p className="mt-1 text-sm text-[var(--text-2)]">{detail.substyle.slug}</p>
              </div>
              <Badge variant={detail.substyle.status === "published" ? "primary" : detail.substyle.status === "ready" ? "success" : detail.substyle.status === "review" ? "warning" : "neutral"}>
                {detail.substyle.status}
              </Badge>
            </div>
          ...
          <Card>
            <CardContent>
              ...
              <form action="/api/admin/substyles/update" method="post" className="grid gap-3 md:grid-cols-2">
                <input type="hidden" name="slug" value={detail.substyle.slug} />
                <input type="hidden" name="redirectTo" value={redirectTo} />
                <div>
                  <label className="mb-1 block text-xs uppercase tracking-[0.08em] text-[var(--text-3)]">Style base</label>
                  <select
                    name="styleSlug"
                    defaultValue={detail.substyle.styleSlug}
                ...
                <Field name="name" label="Nombre" defaultValue={detail.substyle.name} required />
                <Field name="summary" label="Resumen" defaultValue={detail.substyle.summary} required className="md:col-span-2" />
                <Field name="musicalFocus" label="Musical focus" defaultValue={detail.substyle.musicalFocus} />
                <TextAreaField
                  name="historicalCulturalContext"
                  label="Historical/Cultural Context"
                  defaultValue={detail.substyle.historicalCulturalContext}
                  className="md:col-span-2"
                />
                <TextAreaField
                  name="technicalFocus"
                  label="Technical focus (1 linea por item)"
                  defaultValue={detail.substyle.technicalFocus.join("\n")}
                  className="md:col-span-2"
                />
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            <Card>
              <CardContent>
                ...
                <div className="grid gap-2">
                  <StatusActionButton slug={detail.substyle.slug} redirectTo={redirectTo} targetStatus="draft" label="Mover a draft" />
                  <StatusActionButton slug={detail.substyle.slug} redirectTo={redirectTo} targetStatus="review" label="Mover a review" />
                  <StatusActionButton slug={detail.substyle.slug} redirectTo={redirectTo} targetStatus="ready" label="Mover a ready" />
                  <StatusActionButton slug={detail.substyle.slug} redirectTo={redirectTo} targetStatus="published" label="Publicar" />
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent>
                ...
                <form action="/api/admin/substyles/citation" method="post" className="space-y-2">
                  <input type="hidden" name="slug" value={detail.substyle.slug} />
                  <input type="hidden" name="redirectTo" value={redirectTo} />
          ...
          <AdminMediaManager
            entityType="substyle"
            entityRef={detail.substyle.slug}
            redirectTo={redirectTo}
            links={mediaLinks}
            title="Media del substyle"
            description="Administra videos externos y rightsStatus del substyle."
          />
          ...
          <Card>
            <CardContent>
              <form action="/api/admin/substyles/archive" method="post">
                <input type="hidden" name="slug" value={detail.substyle.slug} />
                <input type="hidden" name="redirectTo" value={buildAdminHref("/admin/substyles", role)} />
          ...
          {!detail.canPublish ? (
            <Card className="border-amber-500/20 bg-amber-500/10">
              <CardContent className="flex items-center gap-3 text-sm text-amber-100">
                ...
                El substyle aun no cumple guardrails para publicacion.
              </CardContent>
            </Card>
          ) : null}
        </section>
...
function StatusActionButton({
  ...
  return (
    <form action="/api/admin/substyles/status" method="post">

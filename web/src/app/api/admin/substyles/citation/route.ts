...
import { linkCitationToSubstyleBySlug } from "@/server/db/admin-taxonomy";
import { validateSubstyleCitationInput } from "@/server/validation/admin";
import {
...
export async function POST(request: NextRequest) {
  ...
  const parsedInput = validateSubstyleCitationInput({
  ...
  const result = await linkCitationToSubstyleBySlug({
    ...parsedInput.data,
